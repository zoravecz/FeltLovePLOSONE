%% Fit an Extended Condorcet Model on data from the love study conducted on MTurk
%% If you don't want to run the model just inspect the results go to line 184
%% Preamble
% Cleanup first
clear all
close all
...
% religion      = standardize(fullDataMatrix.data(:,6));
...
nrofgcov = size(Xtheta, 2);
nrofbcov = size(Xtheta, 2);
...
data = struct('Y', Y, 'N', N, ...
    ...
    'nP', P, ...                'P', P, ...
    'nI', I, ...                'Items', I, ...
    'AS_MATRIX_XT', Xtheta, ...           'Xtheta', Xtheta, ...
    'AS_MATRIX_XG', Xg, ...
    'AS_MATRIX_XB', Xb, ...
    'AS_MATRIX_Xdelta', Xdelta, ...
    'nCT', nrofthetacov, ...     'nrofthetacov', nrofthetacov, ...
    'nCB', nrofbcov, ...         'nrofbcov', nrofbcov, ...
    'nCG', nrofgcov, ...
    'nCD', nrofdeltacov); %          'nrofgcov', nrofgcov);
